...
<body class="min-h-screen flex">
    ...
    <div class="flex-1 ml-0 md:ml-64 transition-all duration-300 ease-in-out">
        ...
        <main class="p-6">
            ...
            $allowed_tabs = ['profile','dashboard', 'diagram', 'workers', 'jobs', 'reviews', 'add_worker', 'add_job'];
    ...
    <script>
        ...
        // Edit Worker
        editWorkerBtns.forEach(btn => btn.addEventListener('click', function() { /* ... (Logika sama) ... */ const workerId = this.dataset.workerId; const workers = <?php echo json_encode($workers); ?>; const worker = workers.find(w => w.id_pekerja === workerId); if (worker) { document.getElementById('editWorkerId').value = worker.id_pekerja; document.getElementById('editWorkerName').value = worker.nama; document.getElementById('editWorkerEmail').value = worker.email; document.getElementById('editWorkerPhone').value = worker.telepon; document.getElementById('editWorkerLocation').value = worker.lokasi; document.getElementById('editWorkerRate').value = worker.tarif_per_jam; document.getElementById('editWorkerExperience').value = worker.pengalaman || ''; document.getElementById('editWorkerDescription').value = worker.deskripsi_diri || ''; document.getElementById('editWorkerStatus').value = worker.status_ketersediaan; document.getElementById('currentWorkerPhoto').src = worker.url_foto; document.getElementById('currentPhotoUrl').textContent = worker.url_foto; document.getElementById('editWorkerTitle').textContent = 'Edit: ' + worker.nama; const skillsSelect = document.getElementById('editWorkerSkills'); Array.from(skillsSelect.options).forEach(option => { option.selected = Array.isArray(worker.keahlian) && worker.keahlian.includes(option.value); }); editWorkerModal.classList.remove('hidden'); document.body.style.overflow = 'hidden'; } }));
